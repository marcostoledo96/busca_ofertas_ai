...
- Permisos de archivo restrictivos `0600` en POSIX para la base de datos local.
...
- Transaccionalidad atómica síncrona con rechazo runtime de callbacks async/thenables y control estricto de scope.
...
- **Requisito de Node.js**: Node.js **>= 22.13.0** (versión en la que `node:sqlite` quedó disponible de forma estándar sin necesidad del flag `--experimental-sqlite`).
...
### Configuración de PRAGMAs y Seguridad
...
3. **Permisos de Archivo**: En plataformas POSIX, el archivo SQLite se asegura con permisos `0600` (`-rw-------`) tanto al crearse como al abrir un archivo preexistente.
...
- **Atomicidad ante fallos**: Cada migración se ejecuta dentro de su propia transacción síncrona. Si una sentencia falla o si se retorna un thenable, se hace `ROLLBACK` completo del DDL y la migración **no** queda registrada como aplicada.
...
4. **Conservar permisos**: Mantener permisos restrictivos (por ejemplo `0600` en el archivo o `0700` en el directorio).
