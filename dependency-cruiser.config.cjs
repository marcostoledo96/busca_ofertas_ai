...
module.exports = {
  forbidden: [
    ...
    {
      name: 'core-no-rules-engine',
      comment: 'packages/core no debe depender de rules-engine.',
      severity: 'error',
      from: { path: '^packages/core/' },
      to: { path: '^packages/rules-engine/' },
    },
    {
      name: 'rules-engine-no-adapters',
      comment: 'packages/rules-engine no debe depender de adapters.',
      severity: 'error',
      from: { path: '^packages/rules-engine/' },
      to: { path: '^adapters/' },
    },
    {
      name: 'rules-engine-forbidden-dependencies',
      comment:
        'packages/rules-engine es un evaluador puro y no debe depender de storage-sqlite, report-html, run-export, apps/cli, packages/ai, node:fs, node:http, node:https, node:net, playwright.',
      severity: 'error',
      from: { path: '^packages/rules-engine/' },
      to: {
        path: [
